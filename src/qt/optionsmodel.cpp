...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizegbcoinAmount"))
        settings.setValue("nAnonymizegbcoinAmount", 1000);
    nAnonymizegbcoinAmount = settings.value("nAnonymizegbcoinAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizegbcoinAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizegbcoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizegbcoinAmount:
            return QVariant(nAnonymizegbcoinAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizegbcoinAmount:
            nAnonymizegbcoinAmount = value.toInt();
            settings.setValue("nAnonymizegbcoinAmount", nAnonymizegbcoinAmount);
            emit AnonymizegbcoinAmountChanged(nAnonymizegbcoinAmount);
            break;
        default:
            break;
        }
