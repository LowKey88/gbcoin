...
int nAnonymizegbcoinAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\gbcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\gbcoin
    // Mac: ~/Library/Application Support/gbcoin
    // Unix: ~/.gbcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "gbcoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "gbcoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "gbcoind.pid"));
