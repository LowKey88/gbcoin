...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizegbcoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizegbcoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizegbcoinAmount = strAnonymizegbcoinAmount.remove(strAnonymizegbcoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizegbcoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizegbcoinAmount*COIN) nMaxToAnonymize = nAnonymizegbcoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizegbcoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizegbcoinAmount));
        strAnonymizegbcoinAmount = strAnonymizegbcoinAmount.remove(strAnonymizegbcoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizegbcoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
