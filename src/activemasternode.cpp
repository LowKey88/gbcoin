...
//
// Bootup the masternode, look for a 2500 GBC input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* rewards are not supported in gbcoin.conf */
...
{
    CgbcoincoinAddress address(collateralAddress);
